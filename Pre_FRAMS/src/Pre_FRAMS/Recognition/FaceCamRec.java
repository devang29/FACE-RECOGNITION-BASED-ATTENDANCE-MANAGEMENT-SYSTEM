     ...
     * @author devang
     */
    public class FaceCamRec extends JPanel implements Runnable
    {
        BufferedImage S;
      ...
      private static final int DETECT_DELAY = 100;   
      ...
      private ExecutorService executor,facerec_executor;
      private AtomicInteger numTasks;
           // used to record number of detection tasks
      static private File temp=null;
      ...
      public FaceCamRec(int id)
      {
          this.CAMERA_ID=id;
        setBackground(Color.white);

        executor = Executors.newSingleThreadExecutor();
        facerec_executor=Executors.newSingleThreadExecutor();
    ...
    new Thread(this).start();
      ...
      grabber = initGrabber(CAMERA_ID); //initialize camera
        ...
        while (isRunning) {
          ...
          imageCount++;
          repaint();
          if(!facerec_executor.isShutdown())
          {
facerec_executor.execute(new Runnable(){public void run(){
...
temp.delete();
    ...
    */
    ...
    i++;
        String fnm=temp.getAbsolutePath()+"\\"+"fac"+i+".jpg";
          final String fnm1=fnm;
    try {
              //new File(FACE_DIR).mkdirs();
        new File(fnm).createNewFile();
//Thread.sleep(50);
        ImageIO.write(faceIm, "jpg", new File(fnm));
         
                
         New_FaceThread nf=new New_FaceThread("Library_Image\\Computer Science & Engineering\\6\\2",fnm1);
